  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-lg mx-4 space-y-4">
        ...
        <div className="rounded-2xl p-5 shadow-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        <div className="flex justify-end">
          {canSkip ? (
            <button onClick={handleSkip}
              className="px-6 py-2.5 rounded-xl font-semibold text-sm shadow-lg transition-all flex items-center gap-2"
...
export default function AskQuestion() {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto" dir={dir}>
      <div className="mb-8">
        <h1 className="text-2xl font-bold gradient-text">{t('ask_title')}</h1>
      ...
      <form onSubmit={handleSubmit} className="card space-y-5">
        ...
        {error && (
          <div className="rounded-xl p-3 text-sm" style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171' }}>
            {error}
          </div>
        )}
...
function AnalysisResult({ analysis, consultationId, onReset }: { analysis: Analysis; consultationId: string; onReset: () => void }) {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6 animate-slide-in" dir={dir}>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold gradient-text">{t('ask_done_title')}</h1>
      ...
      <div className="card text-center space-y-3">
        ...
        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
          Experts are being notified and examined. You'll receive a chat invitation when an expert passes.
        </p>
        <div className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>ID: {consultationId}</div>
      </div>
    </div>
  );
}
