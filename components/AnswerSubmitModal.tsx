...
export default function AnswerSubmitModal({ consultationId, consultationTitle, onClose, onSubmitted }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const timeSpentSeconds = Math.floor((Date.now() - openedAt.current) / 1000);
    try {
      const res = await fetch('/api/submissions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ consultationId, content, reasoning, timeSpentSeconds }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" dir="auto">
      <div className="bg-[var(--surface)] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="text-sm font-semibold text-[var(--text-secondary)] mb-1.5 block">Your Answer *</label>
            <textarea
              ...
              className="w-full border border-[var(--border)] rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none bg-[var(--bg)] text-[var(--text-primary)]"
            />
            <div className="text-xs text-[var(--text-muted)] mt-1 text-end">{content.length} characters</div>
          </div>
          <div>
            <label className="text-sm font-semibold text-[var(--text-secondary)] mb-1.5 block">Reasoning & References (optional)</label>
            <textarea
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 py-3 border border-[var(--border)] text-[var(--text-secondary)] rounded-xl font-semibold hover:bg-[var(--surface-2)] transition-all">Cancel</button>
