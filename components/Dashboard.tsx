...
const STAT_CONFIG = [
  { key: 'totalExperts',        gradFrom: '#6366f1', gradTo: '#8b5cf6', glow: 'rgba(99,102,241,0.3)',
  ...
  { key: 'totalDomains',        gradFrom: '#8b5cf6', gradTo: '#ec4899', glow: 'rgba(139,92,246,0.3)',
...
const STEP_ICONS = [
  { grad: ['#6366f1','#8b5cf6'], icon: <path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd"/> },
  { grad: ['#38bdf8','#6366f1'], icon: <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"/> },
  { grad: ['#8b5cf6','#ec4899'], icon: <path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z"/> },
  ...
  { grad: ['#fbbf24','#f59e0b'], icon: <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/> },
];
...
const STATUS_STYLE: Record<string, { bg: string; color: string; border: string }> = {
  ...
  routing:   { bg: 'rgba(167,139,250,0.1)',  color: '#a78bfa', border: 'rgba(167,139,250,0.2)' },
        ...
        <div className="card space-y-3">
          <h2 className="font-bold flex items-center gap-2.5" style={{ color: 'var(--text-primary)' }}>
            <span className="w-7 h-7 rounded-lg flex items-center justify-center text-sm"
              style={{ background: 'linear-gradient(135deg,#f59e0b,#fbbf24)', boxShadow: '0 4px 12px rgba(251,191,36,0.3)' }}>
              🏆
            </span>
            {t('dash_top_experts')}
          </h2>
          {stats.topExperts.map((expert, i) => (
            <div key={expert.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150"
              ...
              <div className="text-sm font-black" style={{ color: '#fbbf24' }}>⭐ {expert.rating.toFixed(1)}</div>
            </div>
          ))}
        </div>
