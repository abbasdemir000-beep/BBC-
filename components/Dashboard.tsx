...
export default function Dashboard() {
  ...
  const statCards = [
    { label: t('dash_experts'),     value: stats.totalExperts,        icon: '🧠', color: 'from-blue-500 to-blue-600' },
    { label: t('dash_domains'),     value: stats.totalDomains,        icon: '📚', color: 'from-purple-500 to-purple-600' },
    { label: t('dash_active'),      value: stats.activeConsultations, icon: '⚡', color: 'from-green-500 to-green-600' },
    { label: t('dash_submissions'), value: stats.totalSubmissions,    icon: '📝', color: 'from-amber-500 to-amber-600' },
  ];

  const steps = [
    { icon: '❓', title: t('step_ask'),     desc: t('step_ask_desc') },
    { icon: '🤖', title: t('step_ai'),      desc: t('step_ai_desc') },
    { icon: '👥', title: t('step_experts'), desc: t('step_experts_desc') },
    { icon: '🔍', title: t('step_eval'),    desc: t('step_eval_desc') },
    { icon: '📝', title: t('step_exam'),    desc: t('step_exam_desc') },
    { icon: '🏆', title: t('step_reward'),  desc: t('step_reward_desc') },
  ];
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-4 gap-4">
        {statCards.map(s => (
          <div key={s.label} className="card flex items-center gap-4">
            <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${s.color} flex items-center justify-center text-2xl shadow-sm flex-shrink-0`}>{s.icon}</div>
            <div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="card">
          <h2 className="font-semibold text-slate-800 mb-4 flex items-center gap-2"><span>🏆</span>{t('dash_top_experts')}</h2>
          <div className="space-y-3">
            {stats.topExperts.map((expert, i) => (
              <div key={expert.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-white text-xs font-bold">{i + 1}</div>
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-400 to-purple-500 flex items-center justify-center text-white text-xs font-bold">{expert.name.charAt(0)}</div>
                ...
                <div className="flex items-center gap-1 text-amber-500 text-sm font-bold">⭐ {expert.rating.toFixed(1)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card">
          <h2 className="font-semibold text-slate-800 mb-4 flex items-center gap-2"><span>⚡</span>{t('dash_recent')}</h2>
          <div className="space-y-3">
            {stats.recentConsultations.map(c => (
              <div key={c.id} className="p-3 rounded-xl hover:bg-slate-50 transition-colors">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  {c.domain && <span className="text-xs text-slate-500">{c.domain.name}</span>}
      ...
      <div className="card">
        <h2 className="font-semibold text-slate-800 mb-6">{t('dash_how')}</h2>
