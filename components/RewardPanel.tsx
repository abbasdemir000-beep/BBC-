    ...
    <div className="p-8 space-y-6" dir={dir}>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="card text-white overflow-hidden" style={{ background: 'linear-gradient(135deg,#f59e0b,#f97316)' }}>
          ...
          <div className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.8)' }}>{t('rew_total_pts')}</div>
        </div>
        <div className="card text-white overflow-hidden" style={{ background: 'linear-gradient(135deg,#10b981,#34d399)' }}>
          <div className="absolute inset-x-0 top-0 h-px" style={{ background: 'rgba(255,255,255,0.3)' }} />
          <div className="text-4xl mb-2">💵</div>
          <div className="text-3xl font-bold">${moneyValue.toFixed(2)}</div>
          <div className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.8)' }}>{t('rew_money')}</div>
        </div>
      ...
      <div className="card">
        ...
        {rewards.length === 0 ? (
          <div className="text-center py-8">
            <div className="text-4xl mb-2">🏅</div>
            <p style={{ color: 'var(--text-muted)' }}>{t('rew_no_rewards')}</p>
          </div>
        ) : (
            ...
            {rewards.map(r => (
              <div key={r.id} className="flex items-center gap-4 p-3 rounded-xl"
                ...
                <div className="text-end">
                  <div className="font-bold" style={{ color: '#fbbf24' }}>+{r.points} {t('pts')}</div>
...
function WithdrawalSection({ totalPoints }: { totalPoints: number }) {
  ...
  return (
    <div className="card space-y-4">
      ...
      <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Minimum 1,000 points ($1.00). Processing takes 3-5 business days.</p>
        ...
        <form onSubmit={handleWithdraw} className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold w-28 text-right" style={{ color: 'var(--accent)' }}>{amount.toLocaleString()} pts</span>
            </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              {(['paypal', 'bank'] as const).map(m => (
                <label key={m} className="flex-1 flex items-center justify-center gap-2 rounded-xl p-3 cursor-pointer transition-all"
                  style={method === m ? {
          ...
          <button type="submit" disabled={submitting || totalPoints < 1000}
            className="w-full py-3 text-white rounded-xl font-semibold transition-all disabled:opacity-60"
