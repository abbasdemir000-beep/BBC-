  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-black/50 backdrop-blur-sm">
      <div className="m-auto w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
        ...
        <div className="modal-header px-6 py-4 flex items-center justify-between flex-shrink-0">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center p-12">
            <div className="w-10 h-10 rounded-full border-4 border-t-[#c2714f] animate-spin"
              style={{ borderColor: 'var(--surface-2)', borderTopColor: '#c2714f' }} />
          </div>
              ...
              {submissions.map((s, i) => (
                <button key={s.id}
                  ...
                  style={{
                    ...
                    background: selected?.id === s.id ? 'rgba(194,113,79,0.08)' : 'transparent',
                    borderLeft: selected?.id === s.id ? '4px solid #c2714f' : '4px solid transparent',
                  }}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>
                        {s.expert.domain?.icon} {s.expert.domain?.name}
                      </div>
                    </div>
                  </div>
                  {s.finalScore != null && (
                    <div className="text-right">
                      <span className="text-xs font-bold" style={{ color: '#c2714f' }}>Score: {s.finalScore.toFixed(0)}</span>
                    </div>
                  )}
              ...
              {!selected ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="ms-auto px-3 py-1 rounded-xl text-xs font-semibold"
                      style={{
                        background: selected.status === 'rated' || selected.status === 'winner'
                          ? 'rgba(16,185,129,0.12)' : 'var(--surface-2)',
                        color: selected.status === 'rated' || selected.status === 'winner'
                          ? '#059669' : 'var(--text-muted)',
                      }}>
                      {selected.status}
                    </span>
                  </div>
                  ...
                  {selected.reasoning && (
                    <div className="rounded-2xl p-4"
                      style={{ background: 'rgba(194,113,79,0.06)', border: '1px solid rgba(194,113,79,0.2)' }}>
                      <h3 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#c2714f' }}>
                  ...
                  <div className="card space-y-3">
                    ...
                    <div className="pt-2 flex justify-between items-center"
                      ...
                      <span className="text-2xl font-black" style={{ color: '#c2714f' }}>
                        {selected.aiScore?.toFixed(0) ?? '—'}
                      </span>
                    </div>
                  </div>
                  ...
                  {selected.examResult && (
                    <div className="rounded-2xl p-4"
                      ...
                      <h3 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#059669' }}>
                        Exam Result
                      </h3>
                      <div className="flex items-center gap-3">
                        <span className="text-3xl font-black" style={{ color: '#059669' }}>
                          {selected.examResult.score}%
                        </span>
                        <p className="text-sm" style={{ color: '#059669' }}>{selected.examResult.feedback}</p>
                      </div>
                    </div>
                  )}
                    ...
                    {ratingDone ? (
                      <div className="text-center py-4">
                        ...
                        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                          Thanks for rating! {rating}/5 stars
                        </p>
                      </div>
                    ) : (
                      <div className="flex gap-2 justify-center">
                        {[1, 2, 3, 4, 5].map(star => (
                          <button key={star} disabled={submitting}
                            ...
                            style={{ color: star <= (hovered || rating) ? undefined : 'var(--border)' }}>
                            ⭐
                          </button>
                        ))}
                      </div>
