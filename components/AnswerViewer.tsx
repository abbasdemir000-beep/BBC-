...
export default function AnswerViewer({ consultationId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-black/50 backdrop-blur-sm">
      <div className="m-auto w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="w-64 border-r border-slate-100 overflow-y-auto flex-shrink-0">
              {submissions.map((s, i) => (
                <button key={s.id} onClick={() => { setSelected(s); setRating(s.userRating ?? 0); setRatingDone(!!s.userRating); }}
                  className={`w-full text-left p-4 border-b border-slate-50 hover:bg-slate-50 transition-all ${selected?.id === s.id ? 'bg-brand-50 border-l-4 border-l-brand-500' : ''}`}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-slate-800 truncate">{s.expert.name}</div>
                      <div className="text-xs text-slate-400 truncate">{s.expert.domain?.icon} {s.expert.domain?.name}</div>
                    </div>
                  </div>
                  {s.finalScore != null && (
              ...
              {!selected ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-bold text-slate-900">{selected.expert.name}</div>
                      <div className="text-sm text-slate-500">{selected.expert.domain?.icon} {selected.expert.domain?.name} · ⭐ {selected.expert.rating.toFixed(1)}</div>
                    </div>
                    <div className={`ms-auto px-3 py-1 rounded-xl text-xs font-semibold ${selected.status === 'rated' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                      {selected.status}
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-2xl p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Answer</h3>
                  ...
                  <div className="card space-y-3">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide">AI Evaluation</h3>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                      <span className="text-sm font-semibold text-slate-700">AI Score</span>
                  ...
                  {/* Rating */}
                  <div className="card">
                    <h3 className="text-sm font-semibold text-slate-700 mb-3">Rate this answer</h3>
                    {ratingDone ? (
